...
var _socketHelper = require('./socket-helper');
...
class ModbusMaster {
    constructor(socket, options) {
        ...
        this.socket = _socketHelper.SocketHelperFactory.create(socket, this._options);
    }
    ...
    createFixedPacket(slave, func, param, param2) {
        return new _bufferput2.default().word16be(1) //request
        .word16be(0) //MODBUS protocol
        .word16be(6) //number of following bytes
        .word8be(slave).word8be(func).word16be(param).word16be(param2).buffer();
    }
